...
/**
 * Lettuce-backed Redis BUFFER layer using Strata-maintained registry and index sets.
 *
 * <p>Query support is based on maintained sets and does not use Redis KEYS.</p>
 */
public final class LettuceLayer implements Layer {
    ...
    private LettuceLayer(@NotNull String name, @NotNull RedisCommands<String, String> commands, @NotNull String prefix, int scanCount) {
    ...
    public static @NotNull LettuceLayer create(@NotNull StatefulRedisConnection<String, String> connection) {
        return named("lettuce", connection, "strata:lettuce");
    }

    public static @NotNull LettuceLayer named(@NotNull String name, @NotNull StatefulRedisConnection<String, String> connection, @NotNull String prefix) {
        Objects.requireNonNull(connection, "connection");
        return new LettuceLayer(name, connection.sync(), prefix, DEFAULT_SCAN_COUNT);
    }

    public static @NotNull LettuceLayer named(@NotNull String name, @NotNull RedisCommands<String, String> commands, @NotNull String prefix) {
        return new LettuceLayer(name, commands, prefix, DEFAULT_SCAN_COUNT);
    }

    public @NotNull LettuceLayer withScanCount(int scanCount) {
        if (scanCount <= 0) {
            throw new IllegalArgumentException("scanCount must be positive.");
        }
        return new LettuceLayer(name, commands, prefix, scanCount);
    }
    ...
    private Result<Void> saveNow(Envelope envelope, OperationContext context) {
        ...
        try {
            ...
            Long saved = commands.eval(SAVE_SCRIPT, ScriptOutputType.INTEGER, keys.toArray(String[]::new), args.toArray(String[]::new));
    ...
    private Result<Void> deleteNow(Key<?> key) {
        ...
        try {
            ...
            commands.eval(DELETE_SCRIPT, ScriptOutputType.INTEGER, keys.toArray(String[]::new), key.external());
            return Result.success(null, List.of(LayerReport.success(this, Duration.between(start, Instant.now()))));
        } catch (Exception exception) {
            return failure(start, exception);
        }
    ...
    private Envelope read(String externalKey) throws Exception {
        String value = commands.get(entryKey(externalKey));
        if (value == null) {
            return null;
        }
    ...
    private void scanSet(String set, Consumer<String> consumer) {
        ...
        do {
            ValueScanCursor<String> result = commands.sscan(set, cursor, ScanArgs.Builder.limit(scanCount));
    ...
    private void cleanupCandidate(Type<?> type, Query query, String external) {
        commands.srem(typeSet(type), external);
        if (query.operator() == Query.Operator.EQ) {
            commands.srem(indexSet(type, query.field(), query.value()), external);
        }
    }
